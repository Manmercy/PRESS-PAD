...
- Shelf uses uniform spine sizing, white title/artist labels, compact sharp cover thumbnails, and a full-spine crop of each canonical cover with a subtle 3px blur. Only the decorative background is blurred. A dark overlay keeps white labels readable; 12 stable track-specific colors remain as loading/error fallbacks. Existing WebP images are reused without adding image files. Catalog numbers remain stable when collection order changes. Mobile keeps a horizontal scrollable shelf with visible focus and touch-sized tape buttons.
